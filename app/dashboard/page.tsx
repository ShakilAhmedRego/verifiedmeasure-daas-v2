...
export default function DashboardPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const entitledIds = new Set<string>((accessData ?? []).map((r: LeadAccessRow) => r.lead_id));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-2xl border border-neutral-800 bg-neutral-950 p-5 md:col-span-2">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <input
              ...
              placeholder="Search company, email, industry, meta…"
            />
          </div>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  {filtered.map((l) => {
                    ...
                    const company = isEntitled ? l.company : maskCompany(l.company);
                    const email = isEntitled ? (l.email || "—") : maskEmail(l.email);
                    const phone = isEntitled ? (l.phone || "—") : maskPhone(l.phone);

                    const industry = safeMetaString(l.meta, "industry");
                    const timeZone = safeMetaString(l.meta, "time_zone");
                    ...
                    return (
                      <tr key={l.id} className="border-b border-neutral-900 hover:bg-neutral-950/40">
                        ...
                        <td className="px-3 py-2 text-neutral-300">{email}</td>
                        <td className="px-3 py-2 text-neutral-300">{phone}</td>
                        <td className="px-3 py-2 text-neutral-300">{industry}</td>
                        <td className="px-3 py-2 text-neutral-300">{timeZone}</td>
        ...
        {/* Right: Entitlement actions */}
        <div className="rounded-2xl border border-neutral-800 bg-neutral-950 p-5">
          <div className="text-sm font-semibold">Entitlement</div>
          <div className="mt-2 text-sm text-neutral-400">
            Claim creates entitlement records (<span className="font-mono">lead_access</span>). Cost applies only to NEW entitlement inserts.
          </div>
          ...
          <div className="mt-6 rounded-2xl border border-neutral-800 bg-neutral-950 p-4 text-xs text-neutral-500">
            ...
            <ul className="mt-2 list-disc space-y-1 pl-5">
              ...
              <li>Masking: company/email/phone remain blurred until entitled.</li>
            </ul>
          </div>
        </div>
      </div>
